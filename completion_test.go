...
func TestRunCompletionBashNonEmpty(t *testing.T) {
	...
	for _, want := range []string{"complete -F", "signup", "share", "remove", "completion", "dashboard", "dash", "--d", "--visibility", "--comments", "--private", "--no-comments"} {
...
func TestRunCompletionZshNonEmpty(t *testing.T) {
	...
	for _, want := range []string{"#compdef gander", "_gander", "signup", "share", "remove", "completion", "dashboard", "dash", "--d", "--visibility", "--comments", "--private", "--no-comments"} {
		if !strings.Contains(out, want) {
