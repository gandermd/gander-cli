...
When gandering a file (not a directory), pick gander share --silent vs gander watch --silent from the same classification table: reports as a static share; plans, RFCs, and drafts live-watched. Do not send --label <type> (the CLI stamps the type label). Extra tags (--label review) are fine. Never --no-labels unless the user asked to clear.
