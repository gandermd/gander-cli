...
When gandering a file (not a directory), pick gander share --silent vs gander watch --silent from the same classification table: reports as a static share; plans, RFCs, and drafts live-watched. Do not send --label <type> or --label review (the CLI stamps review on --silent shares and dir-adopted files, plus the type label). Never --no-labels unless the user asked to clear.
...
- Yes → gander watch <abs-dir> (hosted if signed up; otherwise gander --watch <abs-dir>). Directory watch is silent by design; do not add --silent. Then start the usual comment-poll window, because this is a gander. New files are classified (reports as a static share, plans/RFCs/drafts live-watched) and get review plus a type label. Do not double-gander a dir-watched tree.
...
func dispatchMCPTool(cli *apiClient, cfg Config, name string, args json.RawMessage) (string, error) {
	...
	case "gander_list_comments":
		...
		if strings.TrimSpace(in.Path) == "" {
			...
			return marshalInboxSummary(items, poll), nil
		}
